 ...
 private:
  ...
  static void *raw_handler(void *arg) {
    try {
      ...
      std::string message{"std::exception caught in jthread handler - "};
      message += e.what();
  ...
  while (!stopped_) {
    ...
    if (!stopped_) {
      DBUG_EXECUTE_IF("masking_functions_signal_on_cache_reload", {
        const char act[] = "now WAIT_FOR masking_functions_before_cache_reload";
      ...
      try {
        ...
        std::string message{
            "std::exception caught during reloading dictionary cache - "};
        message += e.what();
        LogComponentErr(ERROR_LEVEL, ER_LOG_PRINTF_MSG, message.c_str());
      }

      DBUG_EXECUTE_IF("masking_functions_signal_on_cache_reload", {
        DBUG_SET("-d,masking_functions_signal_on_cache_reload");
        const char act[] = "now SIGNAL masking_functions_after_cache_reload";
        assert(!debug_sync_set_action(current_thd, STRING_WITH_LEN(act)));
      });
    }
  }
