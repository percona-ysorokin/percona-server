...
NdbImportUtil::NdbImportUtil() : m_util(*this), c_stats(*this) {
  log_debug(1, "ctor");
...
NdbOut &operator<<(NdbOut &out,
                   const NdbImportUtil::DebugLogger::MessageStart &start) {
...
NdbOut &operator<<(NdbOut &out,
                   const NdbImportUtil::DebugLogger::MessageStop &stop) {
...
void NdbImportUtil::Lockable::lock() { NdbMutex_Lock(m_mutex); }

void NdbImportUtil::Lockable::unlock() { NdbMutex_Unlock(m_mutex); }

void NdbImportUtil::Lockable::wait(uint timeout) {
  NdbCondition_WaitTimeout(m_condition, m_mutex, timeout);
}

void NdbImportUtil::Lockable::signal() { NdbCondition_Signal(m_condition); }

// thread

NdbImportUtil::Thread::Thread() { m_thread = 0; }

NdbImportUtil::Thread::~Thread() {
  if (m_thread != 0) NdbThread_Destroy(&m_thread);
}

void NdbImportUtil::Thread::join() {
  require(m_thread != 0);
...
NdbImportUtil::List::~List() {
  ListEnt *ent;
  while ((ent = pop_front()) != 0) delete ent;
}

void NdbImportUtil::List::set_stats(Stats &stats, const char *name) {
  {
...
void NdbImportUtil::List::push_back(ListEnt *ent) {
  require(ent != 0);
  ...
  m_totcnt++;
  validate();
  if (m_stat_occup != 0) m_stat_occup->add(m_cnt);
  if (m_stat_total != 0) m_stat_total->add(1);
}

void NdbImportUtil::List::push_front(ListEnt *ent) {
  require(ent != 0);
  require(ent->m_next == 0 && ent->m_prev == 0);
  if (m_cnt == 0) {
    m_front = m_back = ent;
...
void NdbImportUtil::List::push_after(ListEnt *ent1, ListEnt *ent2) {
...
void NdbImportUtil::List::push_before(ListEnt *ent1, ListEnt *ent2) {
  require(ent1 != 0 && ent2 != 0);
  require(ent2->m_next == 0 && ent2->m_prev == 0);
...
NdbImportUtil::ListEnt *NdbImportUtil::List::pop_front() {
  ListEnt *ent = 0;
  if (m_cnt != 0) {
    if (m_cnt == 1) {
      ent = m_front;
...
void NdbImportUtil::List::remove(ListEnt *ent) {
  ListEnt *prev = ent->m_prev;
  ListEnt *next = ent->m_next;
  ent->m_prev = 0;
  ent->m_next = 0;
  if (prev != 0) prev->m_next = next;
  if (next != 0) next->m_prev = prev;
  if (m_front == ent) m_front = next;
  if (m_back == ent) m_back = prev;
  require(m_cnt != 0);
  m_cnt--;
  validate();
  if (m_stat_occup != 0) m_stat_occup->add(m_cnt);
...
NdbImportUtil::ListEnt *NdbImportUtil::List::pop_back() {
  ListEnt *ent = m_back;
  remove(ent);
  return ent;
}

void NdbImportUtil::List::push_back_from(List &src) {
  if (src.m_cnt != 0) {
    if (m_cnt != 0) {
      ListEnt *ent1 = m_back;
    ...
    if (m_maxcnt < m_cnt) m_maxcnt = m_cnt;
    m_totcnt += src.m_cnt;
  }
  validate();
...
void NdbImportUtil::List::validate() const {
  if (m_cnt == 0) {
    require(m_front == 0);
    require(m_back == 0);
  } else {
  ...
  while (ent1 != 0) {
    ...
    if (ent2 != 0) require(ent2->m_next == ent1);
    ent2 = ent1;
    ent1 = ent1->m_next;
    cnt++;
  }
...
void NdbImportUtil::Attr::set_value(Row *row, const void *data,
                                    uint len) const {
  require(!m_isblob);
  require(data != 0);
  ...
  memcpy(p, data, len);
  if (m_pad) memset(&p[totlen], m_padchar, m_size - totlen);
  if (m_nullable) set_null(row, false);
}

void NdbImportUtil::Attr::set_blob(Row *row, const void *data, uint len) const {
  require(m_isblob);
  require(data != 0);
...
void NdbImportUtil::Attr::set_null(Row *row, bool null) const {
...
const uchar *NdbImportUtil::Attr::get_value(const Row *row) const {
  const uchar *p = &row->m_data[m_offset];
  return p;
}

void NdbImportUtil::Attr::get_value(const Row *row, uint32 &value) const {
...
bool NdbImportUtil::Attr::ai_value_not_provided(const Row *row) const {
  const uchar *p = get_value(row);
...
void NdbImportUtil::Attr::get_value(const Row *row, uint64 &value) const {
  const uchar *p = get_value(row);
...
void NdbImportUtil::Attr::get_value(const Row *row, char *buf,
                                    uint bufsz) const {
  const uchar *p = get_value(row);
  memset(buf, 0, bufsz);
...
bool NdbImportUtil::Attr::get_null(const Row *row) const {
  bool null = false;
  if (m_nullable) {
...
uint NdbImportUtil::Attr::get_blob_parts(uint len) const {
  require(m_isblob);
  uint parts = 0;
  if (len > m_inlinesize) {
...
void NdbImportUtil::Attr::set_sqltype() {
  m_sqltype[0] = 0;
  switch (m_type) {
    case NdbDictionary::Column::Tinyint:
...
void NdbImportUtil::Table::add_pseudo_attr(const char *name,
...
const NdbImportUtil::Attr &NdbImportUtil::Table::get_attr(
    const char *attrname) const {
  uint i = 0;
  const uint n = m_attrs.size();
  while (i < n) {
    if (strcmp(m_attrs[i].m_attrname.c_str(), attrname) == 0) break;
    i++;
  }
  require(i < n);
  return m_attrs[i];
...
uint NdbImportUtil::Table::get_nodeid(uint fragid) const {
  require(fragid < m_fragments.size());
  uint nodeid = m_fragments[fragid];
  return nodeid;
}

int NdbImportUtil::add_table(NdbDictionary::Dictionary *dic,
                             const NdbDictionary::Table *tab, uint &tabid,
                             Error &error) {
  require(tab != 0);
  ...
  do {
    ...
    for (uint i = 0; i < attrcnt && ok; i++) {
      ...
      if (attr_auto_inc) {
        if (table.m_autoIncAttrId != Inval_uint && table.m_autoIncAttrId != i) {
          set_error_usage(error, __LINE__,
                          "Table %s : "
      ...
      switch (attr.m_type) {
        ...
        case NdbDictionary::Column::Longvarchar:
          ...
          uint mbmaxlen;
          mbmaxlen = attr.m_charset->mbmaxlen;
          require(mbmaxlen != 0);
      ...
      if (attr.m_pk) {
        ...
        if (strcmp(attr.m_attrname.c_str(), "$PK") == 0) {
          if (i + 1 == attrcnt && nkey == 1 &&
            ...
            set_error_usage(error, __LINE__,
                            "column %u: "
                            "invalid use of reserved column name $PK",
                            i);
            ok = false;
            break;
          }
    ...
    const NdbRecord *keyrec = dic->createRecord(
        tab, speclist, nkey, sizeof(NdbDictionary::RecordSpecification));
    if (keyrec == 0) {
...
const NdbImportUtil::Table &NdbImportUtil::get_table(uint tabid) {
  std::map<uint, Table>::const_iterator it;
  it = c_tables.m_tables.find(tabid);
...
void NdbImportUtil::remove_table(NdbDictionary::Dictionary *dic, uint tabid) {
  std::map<uint, Table>::const_iterator it;
  it = c_tables.m_tables.find(tabid);
  require(it != c_tables.m_tables.end());
  const Table &table = it->second;
...
void NdbImportUtil::Row::init(const Table &table) {
...
void NdbImportUtil::RowList::set_stats(Stats &stats, const char *name) {
...
bool NdbImportUtil::RowList::push_back(Row *row) {
  bool ret = false;
  if (m_cnt < m_rowbatch && m_rowsize < m_rowbytes) {
    ...
    m_overflow++;
    if (m_stat_overflow != 0) m_stat_overflow->add(1);
  }
  return ret;
}

void NdbImportUtil::RowList::push_back_force(Row *row) {
  List::push_back(row);
  m_rowsize += row->m_rowsize;
}

bool NdbImportUtil::RowList::push_front(Row *row) {
  bool ret = false;
  if (m_cnt < m_rowbatch && m_rowsize < m_rowbytes) {
    List::push_front(row);
    m_rowsize += row->m_rowsize;
...
void NdbImportUtil::RowList::push_front_force(Row *row) {
  List::push_front(row);
  m_rowsize += row->m_rowsize;
}

NdbImportUtil::Row *NdbImportUtil::RowList::pop_front() {
  Row *row = 0;
  do {
    ...
    m_underflow++;
    if (m_stat_underflow != 0) m_stat_underflow->add(1);
  } while (0);
  return row;
}

void NdbImportUtil::RowList::remove(Row *row) {
  List::remove(row);
  require(m_rowsize >= row->m_rowsize);
  m_rowsize -= row->m_rowsize;
}

void NdbImportUtil::RowList::push_back_from(RowList &src) {
 ...
 * has fewer rows.
 */
void NdbImportUtil::RowList::push_back_from(RowList &src, RowCtl &ctl) {
  uint retries = ctl.m_retries;
  uint cnt_out = 0;
  uint bytes_out = 0;
  if (unlikely(full())) return;
  while (src.empty() && retries != 0) {
    if (ctl.m_dowait) src.wait(ctl.m_timeout);
    retries--;
  }
  while (!src.empty()) {
 ...
 * has more rows.
 */
void NdbImportUtil::RowList::pop_front_to(RowList &dst, RowCtl &ctl) {
  uint retries = ctl.m_retries;
  uint cnt_out = 0;
  uint bytes_out = 0;
  if (unlikely(empty())) return;
  while (dst.full() && retries != 0) {
    if (ctl.m_dowait) dst.wait(ctl.m_timeout);
    retries--;
  }
  while (!empty()) {
    // pop because row cannot be on 2 lists
...
#if defined(VM_TRACE) || defined(TEST_NDBIMPORTUTIL)
void NdbImportUtil::RowList::validate() const {
  List::validate();
  if (m_cnt == 0) require(m_rowsize == 0);
  if (m_rowsize == 0) require(m_cnt == 0);
#if defined(VM_TRACE) && defined(TEST_NDBIMPORTUTIL)
  uint rowsize = 0;
...
NdbImportUtil::Row *NdbImportUtil::alloc_row(const Table &table, bool dolock) {
  RowList &rows = *c_rows_free;
  if (dolock) rows.lock();
  Row *row = rows.pop_front();
  if (dolock) rows.unlock();
  if (row == 0) {
    row = new Row;
  }
  row->init(table);
...
void NdbImportUtil::alloc_rows(const Table &table, uint cnt, RowList &dst) {
  RowList &rows = *c_rows_free;
  rows.lock();
...
void NdbImportUtil::free_blobs_from_row(Row *row) {
...
void NdbImportUtil::free_row(Row *row) {
...
NdbImportUtil::Blob *NdbImportUtil::alloc_blob() {
...
void NdbImportUtil::free_blob(Blob *blob) {
  BlobList &blobs = *c_blobs_free;
  blobs.lock();
...
NdbImportUtil::RowMap::RowMap(NdbImportUtil &util) : m_util(util) {}
...
void NdbImportUtil::Range::copy(const Range &range2) {
 ...
 */
void NdbImportUtil::RowMap::add(const RowMap &map2) {
  RangeList &ranges = m_ranges;
    ...
    if (r->m_start < r2->m_start) {
      ...
      if (merge_up(r, r2)) {
        ...
        // leave r unchanged as next r2 may also apply
        {
          ...
          if (r2next != 0) require(r->m_end <= r2next->m_start);
        }
      } else {
...
NdbImportUtil::Range *NdbImportUtil::RowMap::find(uint64 rowid) {
  RangeList &ranges = m_ranges;
  Range *r = ranges.front();
  while (r != 0) {
    if (r->m_start <= rowid && rowid < r->m_end) break;
    r = r->next();
  }
  return r;
}

bool NdbImportUtil::RowMap::remove(uint64 rowid) {
  RangeList &ranges = m_ranges;
  Range *r = find(rowid);
  if (r == 0) return false;
  if (rowid == r->m_start) {
...
void NdbImportUtil::RowMap::get_total(uint64 &rows, uint64 &reject) const {
  uint64 trows = 0;
  uint64 treject = 0;
...
NdbImportUtil::Range *NdbImportUtil::alloc_range(bool dolock) {
  RangeList &ranges = c_ranges_free;
  if (dolock) ranges.lock();
  Range *range = ranges.pop_front();
  if (dolock) ranges.unlock();
  if (range == 0) {
    range = new Range;
  }
  return range;
...
void NdbImportUtil::alloc_ranges(uint cnt, RangeList &dst) {
  RangeList &ranges = c_ranges_free;
  ranges.lock();
  for (uint i = 0; i < cnt; i++) {
...
void NdbImportUtil::free_range(Range *range) {
  RangeList &ranges = c_ranges_free;
  ranges.lock();
...
void NdbImportUtil::free_ranges(RangeList &src) {
  RangeList &ranges = c_ranges_free;
  ranges.lock();
...
#if defined(VM_TRACE) || defined(TEST_NDBIMPORTUTIL)
void NdbImportUtil::RowMap::validate() const {
  m_ranges.validate();
#if defined(VM_TRACE) && defined(TEST_NDBIMPORTUTIL)
  ...
  while (r1 != 0) {
    ...
    if (r2 != 0) require(r2->m_end < r1->m_start);
    r2 = r1;
    r1 = r1->next();
  }
#endif
...
NdbOut &operator<<(NdbOut &out, const NdbImportUtil::Range &range) {
  out << "start=" << range.m_start << " end=" << range.m_end
      << " rows=" << range.m_end - range.m_start
      << " startpos=" << range.m_startpos << " endpos=" << range.m_endpos
...
void NdbImportUtil::add_pseudo_tables() {
  add_result_table();
  add_reject_table();
  add_rowmap_table();
  add_stopt_table();
...
void NdbImportUtil::add_result_table() {
  ...
  table.add_pseudo_attr("name", NdbDictionary::Column::Varchar, 10);
  table.add_pseudo_attr("desc", NdbDictionary::Column::Varchar, 100);
  table.add_pseudo_attr("result", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("rows", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("reject", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("temperrors", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("runtime", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("utime", NdbDictionary::Column::Bigunsigned);
  add_error_attrs(table);
}

void NdbImportUtil::add_reject_table() {
  Table &table = c_reject_table;
  table.m_tabid = g_reject_tabid;
  require(table.m_recsize == 0);
  table.add_pseudo_attr("runno", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("rowid", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("linenr", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("startpos", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("endpos", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("bytes", NdbDictionary::Column::Bigunsigned);
  add_error_attrs(table);
...
void NdbImportUtil::add_rowmap_table() {
  ...
  require(table.m_recsize == 0);
  table.add_pseudo_attr("runno", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("start", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("end", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("rows", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("startpos", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("endpos", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("bytes", NdbDictionary::Column::Bigunsigned);
...
void NdbImportUtil::add_stopt_table() {
  Table &table = c_stopt_table;
  table.m_tabid = g_stopt_tabid;
  require(table.m_recsize == 0);
  table.add_pseudo_attr("runno", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("option", NdbDictionary::Column::Varchar, 100);
  table.add_pseudo_attr("value", NdbDictionary::Column::Unsigned);
}

void NdbImportUtil::add_stats_table() {
  Table &table = c_stats_table;
  table.m_tabid = g_stats_tabid;
  require(table.m_recsize == 0);
  table.add_pseudo_attr("runno", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("id", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("name", NdbDictionary::Column::Varchar, 100);
  table.add_pseudo_attr("parent", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("obs", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("sum", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("mean", NdbDictionary::Column::Double);
  table.add_pseudo_attr("min", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("max", NdbDictionary::Column::Bigunsigned);
  table.add_pseudo_attr("stddev", NdbDictionary::Column::Double);
}

#include <math.h>

void NdbImportUtil::add_error_attrs(Table &table) {
  table.add_pseudo_attr("errortype", NdbDictionary::Column::Varchar, 10);
  table.add_pseudo_attr("errorcode", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("sourceline", NdbDictionary::Column::Unsigned);
  table.add_pseudo_attr("errortext", NdbDictionary::Column::Longvarchar, 1024);
}

void NdbImportUtil::set_result_row(Row *row, uint32 runno, const char *name,
                                   const char *desc, uint64 rows, uint64 reject,
                                   uint64 temperrors, uint64 runtime,
                                   uint64 utime, const Error &error) {
...
void NdbImportUtil::set_reject_row(Row *row, uint32 runno, const Error &error,
                                   const char *reject, uint rejectlen) {
...
void NdbImportUtil::set_rowmap_row(Row *row, uint32 runno, const Range &range) {
...
void NdbImportUtil::set_stopt_row(Row *row, uint32 runno, const char *option,
                                  uint32 value) {
...
void NdbImportUtil::set_stats_row(Row *row, uint32 runno, const Stat &stat,
                                  bool global) {
  ...
  // mean
  {
    const Attr &attr = attrs[id];
    double mean = 0.0;
    if (stat.m_obs != 0) mean = sum1 / obsf;
...
void NdbImportUtil::set_error_attrs(Row *row, const Table &table,
                                    const Error &error, uint &id) {
  const Attrs &attrs = table.m_attrs;
  // errortype
  {
    const Attr &attr = attrs[id];
...
NdbImportUtil::Buf::Buf(bool split) : m_split(split) {
  m_allocptr = 0;
  m_allocsize = 0;
  m_data = 0;
  m_size = 0;
...
NdbImportUtil::Buf::~Buf() { delete[] m_allocptr; }

void NdbImportUtil::Buf::alloc(uint pagesize, uint pagecnt) {
...
void NdbImportUtil::Buf::copy(const uchar *src, uint len) {
...
int NdbImportUtil::Buf::movetail(Buf &dst) {
...
void NdbImportUtil::pretty_print(char *dst, const void *ptr, uint len) {
...
int NdbImportUtil::File::do_open(int flags) {
  ...
  if (fd == -1) {
    ...
    if (flags == Read_flags) type = "read";
    if (flags == Write_flags) type = "write";
    if (flags == Append_flags) type = "append";
    m_util.set_error_os(m_error, __LINE__, "%s: open for %s failed", path,
                        type);
    return -1;
  }
  m_fd = fd;
...
int NdbImportUtil::File::do_read(uchar *dst, uint size, uint &len) {
  const char *path = get_path();
  require(m_fd != -1);
  len = 0;
  while (len < size) {
...
#endif
    if (ret == -1) {
      m_util.set_error_os(m_error, __LINE__, "%s: read %u bytes failed", path,
                          size - len);
      return -1;
    }
    uint n = (uint)ret;
...
int NdbImportUtil::File::do_read(Buf &buf) {
  ...
  uint len = 0;
  if (do_read(dst, size, len) == -1) return -1;
  buf.m_eof = (len == 0);
  buf.m_len += len;
  ...
  if (buf.m_data[endpos - 1] == '\r') {
    if (do_read(&buf.m_data[endpos], 1, len) == -1) return -1;
    buf.m_len += 1;
  } else
    buf.m_data[endpos] = 0;
  return 0;
...
int NdbImportUtil::File::do_write(const uchar *src, uint size) {
  const char *path = get_path();
  require(m_fd != -1);
#ifndef _WIN32
...
#endif
  if (ret == -1) {
    m_util.set_error_os(m_error, __LINE__, "%s: write %u bytes failed", path,
                        size);
    return -1;
  }
  // short write is considered error
  if (uint(ret) != size) {
    m_util.set_error_os(m_error, __LINE__, "%s: short write %u < %u", path,
                        (uint)ret, size);
    return -1;
  }
  return 0;
...
int NdbImportUtil::File::do_write(const Buf &buf) {
  const uchar *src = &buf.m_data[buf.m_start];
  uint len = buf.m_len;
  if (do_write(src, len) == -1) return -1;
  return 0;
}

int NdbImportUtil::File::do_close() {
  const char *path = get_path();
  if (m_fd == -1) return 0;
#ifndef _WIN32
  if (::close(m_fd) == -1)
...
#endif
  {
    m_util.set_error_os(m_error, __LINE__, "%s: close failed", path);
    return -1;
  }
  m_fd = -1;
  return 0;
...
int NdbImportUtil::File::do_seek(uint64 offset) {
  const char *path = get_path();
  require(m_fd != -1);
#ifndef _WIN32
...
#endif
  {
    m_util.set_error_os(m_error, __LINE__, "%s: lseek %" PRIu64 " failed", path,
                        offset);
    return -1;
  }
  return 0;
...
NdbImportUtil::Stat::Stat(Stats &stats, uint id, const char *name, uint parent,
                          uint level, uint flags)
    : m_stats(stats),
      m_id(id),
      m_name(name),
      m_parent(parent),
...
void NdbImportUtil::Stat::add(uint64 val) {
  uint id = m_id;
  do {
    ...
    if (stat->m_obs == 1) {
      ...
      if (stat->m_min > val) stat->m_min = val;
      if (stat->m_max < val) stat->m_max = val;
    }
...
NdbImportUtil::Stats::Stats(NdbImportUtil &util) : m_util(util) {
...
NdbImportUtil::Stat *NdbImportUtil::Stats::create(const char *name, uint parent,
                                                  uint flags) {
  lock();
  {
    Stat *stat = find(name);
...
NdbImportUtil::Stat *NdbImportUtil::Stats::get(uint i) {
...
const NdbImportUtil::Stat *NdbImportUtil::Stats::get(uint i) const {
  require(i < m_stats.size());
...
NdbImportUtil::Stat *NdbImportUtil::Stats::find(const char *name) const {
  for (uint i = 0; i < m_stats.size(); i++) {
    Stat *stat = m_stats[i];
    require(stat != 0);
    if (strcmp(stat->m_name, name) == 0) return stat;
  }
  return 0;
}
...
const NdbImportUtil::Stat *NdbImportUtil::Stats::next(uint id) const {
...
void NdbImportUtil::Stats::reset() {
  for (uint i = 0; i < m_stats.size(); i++) {
    Stat *stat = get(i);
    stat->reset();
  }
...
#if defined(VM_TRACE) || defined(TEST_NDBIMPORTUTIL)
void NdbImportUtil::Stats::validate() const {
  bool *seen = new bool[m_stats.size()];
  Validate v(StatNULL, 0, 0, seen);
  for (uint i = 0; i < m_stats.size(); i++) v.m_seen[i] = false;
  (void)validate(v);
  for (uint i = 0; i < m_stats.size(); i++) require(v.m_seen[i] == true);
  delete[] seen;
}

const NdbImportUtil::Stat *NdbImportUtil::Stats::validate(Validate &v) const {
...
void NdbImportUtil::Timer::start() { m_start = NdbTick_getCurrentTicks(); }

void NdbImportUtil::Timer::stop() {
...
uint64 NdbImportUtil::Timer::elapsed_sec() const {
  return NdbTick_Elapsed(m_start, m_stop).seconds();
}

uint64 NdbImportUtil::Timer::elapsed_msec() const {
  return NdbTick_Elapsed(m_start, m_stop).milliSec();
}

uint64 NdbImportUtil::Timer::elapsed_usec() const {
  return NdbTick_Elapsed(m_start, m_stop).microSec();
}

// error

void NdbImportUtil::set_error_gen(Error &error, int line, const char *fmt,
                                  ...) {
  c_error_lock.lock();
  new (&error) Error;
  error.line = line;
  ...
  log_debug(1, "E " << error);
  if (c_opt.m_abort_on_error) abort();
  c_error_lock.unlock();
}

void NdbImportUtil::set_error_usage(Error &error, int line, const char *fmt,
                                    ...) {
  c_error_lock.lock();
  new (&error) Error;
  error.line = line;
...
void NdbImportUtil::set_error_alloc(Error &error, int line) {
  c_error_lock.lock();
  new (&error) Error;
  error.line = line;
...
void NdbImportUtil::set_error_mgm(Error &error, int line, NdbMgmHandle handle) {
  c_error_lock.lock();
  new (&error) Error;
  error.line = line;
...
void NdbImportUtil::set_error_con(Error &error, int line,
...
void NdbImportUtil::set_error_ndb(Error &error, int line,
...
void NdbImportUtil::set_error_os(Error &error, int line, const char *fmt, ...) {
  c_error_lock.lock();
  new (&error) Error;
  error.line = line;
  ...
  }
  {
    ...
    if (len < sizeof(error.text)) {
      ...
      snprintf(&error.text[len], left, ": errno=%u: %s", errno,
               strerror(errno));
    }
  }
  log_debug(1, "E " << error);
...
void NdbImportUtil::set_error_data(Error &error, int line, int code,
                                   const char *fmt, ...) {
  c_error_lock.lock();
  new (&error) Error;
  error.line = line;
...
void NdbImportUtil::copy_error(Error &error, const Error &error2) {
...
void NdbImportUtil::fmt_msec_to_hhmmss(char *str, uint64 msec) {
  uint sec = msec / 1000;
  uint hh = sec / 3600;
  sec -= hh * 3600;
  uint mm = sec / 60;
...
static int testlist() {
  ...
  for (uint numop = 0; numop < numops; numop++) {
    ...
    if (max_occup < recs.m_cnt) max_occup = recs.m_cnt;
  }
  uint last_occup = recs.m_cnt;
  for (MyRec *rec = static_cast<MyRec *>(recs.m_front); rec != 0;
...
static int testrowmap1() {
  ...
  uint toprowid = 0;
  for (uint k = 0; k < maxrowid; k++) rowexist[k] = false;
  {
    uint start = 0;
    for (uint i = 0; i < numranges; i++) {
      ...
      start = r.m_end;
      for (uint k = r.m_start; k < r.m_end; k++) rowexist[k] = true;
      toprowid = r.m_end;
    }
  }
  ...
  {
    for (uint i = 0; i < numranges; i++) reorder[i] = i;
    for (uint i = 0; i < numranges; i++) {
  ...
  for (uint k = 0; k < maxrowid; k++) rowexist4[k] = rowexist[k];
  while (map4.size() != 0) {
...
static int teststat() {
  ...
  for (uint i = 0; i < stattot; i++) iterseen[i] = false;
  // root is skipped
  ...
  for (uint i = 0; i < stattot; i++) require(iterseen[i] == true);
  require(itercnt == statcnt);
  return 0;
}
...
static bool mycase(const char *name) {
...
#endif
  myseed();
  if (mycase("testlist") && testlist() != 0) return -1;
  if (mycase("testrowlist1") && testrowlist1() != 0) return -1;
  if (mycase("testrowlist2") && testrowlist2() != 0) return -1;
  if (mycase("testrowmap1") && testrowmap1() != 0) return -1;
  if (mycase("testrowmap2") && testrowmap2() != 0) return -1;
  if (mycase("testbuf") && testbuf() != 0) return -1;
  if (mycase("testfile") && testfile() != 0) return -1;
  if (mycase("testprint") && testprint() != 0) return -1;
  if (mycase("teststat") && teststat() != 0) return -1;
  struct ndb_rusage ru;
  require(Ndb_GetRUsage(&ru, false) == 0);
  ndbout << "utime=" << ru.ru_utime / 1000 << " stime=" << ru.ru_stime / 1000
         << " (ms)" << endl;
  return 0;
}
